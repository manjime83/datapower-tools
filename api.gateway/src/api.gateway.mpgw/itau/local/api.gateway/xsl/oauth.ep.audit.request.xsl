...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dp="http://www.datapower.com/extensions" xmlns:date="http://exslt.org/dates-and-times" extension-element-prefixes="dp date" exclude-result-prefixes="dp date">
	...
	<xsl:template match="/">
		...
		<dp:dump-nodes file="concat('vars.oauth.ep.audit.request.', dp:variable('var://service/transaction-rule-type'), '.xml')" nodes="$vars" />
		...
		<dp:dump-nodes file="concat('oauth.ep.audit.request.', dp:variable('var://service/transaction-rule-type'), '.xml')" nodes="$result" />
